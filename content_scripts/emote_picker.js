var emote_picker = (function(){
    ...
    var makeSearchingEmoteList = function(Text){
        ...
        var aliasKeys = Object.keys(EmoteAliasList);
        var aliasMatch = {};

        for(var i = 0 ; i < aliasKeys.length ; i ++){
            var cur_alias = aliasKeys[i];
            
            if(matchLowerCase(cur_alias,Text)){
                for(var j = 0 ; j < EmoteAliasList[cur_alias].length ; j ++){
                    var cur_info = EmoteAliasList[cur_alias][j];
                    aliasMatch[cur_info.id] = cur_info;
                }
            }
        }
        ...
        for(var i = emote_sets.length -1 ; i >= 0 ; i --)
        {
            ...
            for(var j = 0 ; j < EmoteAvailableList['emoticon_sets'][cur_set].length ; j ++){
                ...
                if(aliasMatch[emote_id + ''] != undefined){
                    searchEmote.appendEmote(cur_emote);
                }
            }
        }
